...
export enum AiConsultNameMatchType {
  ...
  /**
   * 문자열로는 못 붙어 **모델이 후보 목록에서 직접 고른** 결과
   * ("옷" → 반팔·긴팔·여성긴팔).
   *
   * 상위어는 등록된 이름과 자모가 하나도 겹치지 않아("옷" vs "반팔") 유사도로는
   * 영원히 안 잡힌다. 임계값을 낮춰서 해결되는 종류가 아니라서 의미를 아는 모델에게
   * 넘긴 단계다. 이 타입의 비율이 곧 2차 호출이 값을 하는지의 지표다.
   */
