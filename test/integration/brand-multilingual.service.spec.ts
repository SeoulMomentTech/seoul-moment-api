...
describe('BrandService Multilingual Integration Tests', () => {
  ...
  describe('getBrandIntroduce with multilingual support', () => {
    it('should return brand in Korean by default', async () => {
      ...
      const { brand, languages } = await testDataFactory.createMultilingualBrand(
        { status: 'NORMAL' },
        {
          name: {
    ...
    it('should return brand in specified language', async () => {
      ...
      const englishResult = await brandService.getBrandIntroduce(brand.id, LanguageCode.ENGLISH);
      ...
      const chineseResult = await brandService.getBrandIntroduce(brand.id, LanguageCode.CHINESE);
    ...
    it('should fallback to Korean when requested language is not available', async () => {
      // Given: Create brand with only Korean content
      const { brand } = await testDataFactory.createMultilingualBrand(
        { status: 'NORMAL' },
        {
          name: {
      ...
      const result = await brandService.getBrandIntroduce(brand.id, LanguageCode.ENGLISH);

      // Then: Should fallback to Korean
      expect(result.name).toBe('서울모먼트');
      expect(result.description).toBe('서울의 특별한 순간들');
    });
    ...
    it('should handle multilingual sections', async () => {
      ...
      const koreanResult = await brandService.getBrandIntroduce(brand.id, LanguageCode.KOREAN);
      ...
      expect(koreanResult.section[0].content).toBe('우리 브랜드의 이야기입니다.');
      ...
      expect(englishResult.section[1].title).toBe('제품 소개'); // Fallback to Korean
    });
    ...
    it('should handle complex multilingual brand with all features', async () => {
      ...
      // Create multilingual texts for sections
      const sectionIds = brand.brandSectionList.map(s => s.id);
      await Promise.all([
        testDataFactory.createMultilingualText(
          'BrandSection',
          sectionIds[0],
      ...
      // When: Get brand introduce in Korean
      const result = await brandService.getBrandIntroduce(brand.id, LanguageCode.KOREAN);

      // Then: Should return complete multilingual brand data
      expect(result.id).toBe(brand.id);
      expect(result.name).toBe('서울모먼트');
      expect(result.description).toBe('서울의 특별한 순간들을 담은 브랜드입니다.');
      expect(result.bannerList).toHaveLength(2);
      expect(result.bannerList).toEqual(['banner1.jpg', 'banner2.jpg']);

      expect(result.section).toHaveLength(2);
      expect(result.section[0].title).toBe('브랜드 스토리');
      expect(result.section[0].content).toBe('우리는 서울의 아름다운 순간들을 포착합니다.');
      expect(result.section[0].imageList).toEqual(['section1-1.jpg', 'section1-2.jpg']);
      
      expect(result.section[1].title).toBe('Our Products'); // English content
      expect(result.section[1].content).toBe(''); // No content in any language
      expect(result.section[1].imageList).toEqual(['section2-1.jpg']);
    });
  });
});