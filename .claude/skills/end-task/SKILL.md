...
Worktree 브랜치에서 작업하는 경우 아래 규칙을 반드시 준수한다:

1. **파일 범위 제한**: worktree 폴더 내에서만 파일을 생성/수정/삭제한다.
2. **Breaking Change 처리**: 기존 endpoint의 response/request 키와 값이 바뀌는 경우:
   - 이전 코드는 `@deprecated` 처리한다.
   - 새롭게 v1 폴더에 변경된 코드를 추가한다.
...
prompt: |
  ...
  7. Worktree 파일 범위 (worktree 폴더 외부 파일 수정 여부)
  8. Worktree Breaking Change 처리 (deprecate + v1 폴더 신규 추가 여부)
...
### Step 3.5: Lint & Prettier 실행

테스트가 모두 통과한 후, 커밋 전에 반드시 lint와 prettier를 실행한다.
...
- lint/format 실행 후 변경된 파일이 있으면 diff를 확인한다.
- 에러가 있으면 수정 후 재실행한다.
